...
import { Film, Monitor, ArrowLeft } from "lucide-react";
...
export default function DisplayPage() {
  ...
  const [currentVideo, setCurrentVideo] = useState<PlayVideoPayload | null>(null);
  ...
  useEffect(() => {
    ...
    socket.on(
      "display:play_video",
      (payload: PlayVideoPayload) => {
        if (!payload?.videoUrl) return;

        setPrevGuests((prev) =>
          [payload.guest, ...prev].slice(0, 10),
        );
